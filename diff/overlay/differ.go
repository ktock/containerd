...
func NewOverlayDiffer(store content.Store) diff.Comparer {
...
func (d *overlayDiffer) Compare(ctx context.Context, lower, upper []mount.Mount, opts ...diff.Opt) (desc ocispecs.Descriptor, err error) {
	...
	desc, _, err = d.tryComputeOverlayBlob(ctx, lower, upper, config.MediaType, config.Reference, config.Compressor)
	log.G(ctx).WithField("digest", desc.Digest).WithError(err).Infof("computed diff with overlay differ")
	return
}
...
func writeOverlayUpperdir(ctx context.Context, w io.Writer, upperdir string, lower []mount.Mount) error {
	emptyLower, err := ioutil.TempDir("", "writeoverlayupper") // empty directory used for the lower of diff view
	if err != nil {
