...
func (s *snapshotter) createSnapshot(ctx context.Context, key, parent string, readonly bool, opts []snapshots.Opt) ([]mount.Mount, error) {
	...
	if errdefs.IsAlreadyExists(err) {
		if target != "" {
			tinfo, err := s.Snapshotter.Stat(ctx, target)
			if err != nil {
				return nil, err
			}
			if tinfo.Kind != snapshots.KindCommitted {
				return nil, errors.Wrap(errdefs.ErrInvalidArgument, "snapshot reference must be committed")
			}
			key = target
			bkey = tinfo.Name
			...
			// TODO: Merge base and tinfo labels
...
func (s *snapshotter) Commit(ctx context.Context, name, key string, opts ...snapshots.Opt) error {
	...
	if err := update(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if bname != "" {
			log.G(ctx).WithField("snapshotter", s.name).WithField("key", key).WithField("bname", bname).WithError(err).Error("uncommittable snaphost: transaction failed after commit, snapshot should be removed")
...
// a prefix of "containerd.io/snapshot/" or is "containerd.io/snapshot.ref" .
